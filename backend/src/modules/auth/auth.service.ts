...
export class AuthService {
  constructor(private prisma: PrismaService, private settingsService: SettingsService, private emailService: EmailService) {
    
  }

  async ownerSignup(req, res: any) {
    ...
    try {
      ...
      // 1. Strict Unique Email Check in Restaurants & Users
      ...
      // 2. Strict Unique Phone Check in Restaurants & Users
      ...
      // Send OTP to email via pure Nodemailer SMTP
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        const user = await tx.user.create({
          data: {
            ...
            name: name,
            role: "owner",
            loginId: normalizedEmail, // Owner loginId = their email address
            phone: normalizedPhone,
...
// 2.5 Resend Email OTP
async resendOtp(req, res: any) {
...
async forgotPassword(req, res: any) {
  const { email } = req.body;
...
async resetPassword(req, res: any) {
...
async updateStaffStatus(req, res: any) {
  const { id } = req.params;
